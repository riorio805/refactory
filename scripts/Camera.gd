...
@export var idle_rotation_speed = 60
@export var rotation_speed = 3 * 60
@export var movement_speed = 0.2 * 60

@export var mouse_sensitivity = 0.7
@export var dist_smoothness_mod = 0.12
@export var angle_smoothness_mod = 0.10
...
func _ready():
	...
	distance_step = (float)(max_distance - min_distance)
	angle = Vector2((float)(max_angle + min_angle) / 2, 0)
	real_angle = angle
	angle_step = (float)(max_angle - min_angle)
...
func update_camera():
	...
	var next_camera_pos = Vector3(
		-sin(angle_to_camera.y),
		 0,
		 cos(angle_to_camera.y)
	).normalized() * real_distance
	var rotation_axis = Vector3.RIGHT\
		.rotated(Vector3.UP, - angle_to_camera.y)\
		.normalized()
...
func move_focus(delta):
	...
	var focus_movement = Vector3(movement.x, 0, movement.y)\
			.rotated(Vector3.UP, angle_from_camera.y)
...
func update_real_values():
	...
	if  (angle.y > 360 and real_angle.y > 360) or\
		(angle.y < -360 and real_angle.y < -360):
