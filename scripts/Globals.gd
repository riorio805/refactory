...
enum PlayState { SELECT, BUILD, DELETE }

enum Orientation { PLUS_X, PLUS_Z, MINUS_X, MINUS_Z }

var warnings = [["Hello", 1]]
...
func get_max_square_size(pos: Vector2i):
	...
	for i in range(9):
		for j in range(9):
			...
			if not tiles[x][y] or tiles[x][y].type == GameTileData.TileType.Stone:
				size = min(size, dist)
	return size
